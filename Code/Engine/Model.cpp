...
sad::Model::Model(char* path)
{
	MLoadModel(path);
}

void sad::Model::Draw(sad::rad::ShaderResource& shader)
{
	for (unsigned int i = 0; i < m_Meshes.size(); i++)
		m_Meshes[i].Draw(shader);
}

void sad::Model::MLoadModel(std::string path)
{
...
sad::rad::MeshResource sad::Model::MProcessMesh(aiMesh* mesh, const aiScene* scene)
{
...
std::vector<sad::rad::MeshResource::Texture> sad::Model::MLoadMaterialTextures(aiMaterial* mat, aiTextureType type, std::string typeName)
{
	std::vector<sad::rad::MeshResource::Texture> textures;
