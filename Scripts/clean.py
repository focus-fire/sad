...
"""
def clean():
    bet_log(FILE_NAME, "Cleaning premake build files and metadata.")

    # Intermediate and build directories that may build regardless of toolset 
    bet_log(FILE_NAME, "Checking for intermediate directories...")
    ...
    bet_log(FILE_NAME, "bet! Successfully cleaned premake build files and output.")
...
"""
def rm_vs_files():
    bet_log(FILE_NAME, "Checking for local solution settings...")
    vs_dir = os.path.join(PROJECT_DIR, ".vs")
    rdf(vs_dir)

    bet_log(FILE_NAME, "Cleaning auto-generated solution files...")
    sln_file = os.path.join(PROJECT_DIR, "sad.sln")
    rdf(sln_file)

    bet_log(FILE_NAME, "Cleaning Visual Studio project files...")
    proj_ext = ".vcxproj"
...
"""
def rm_makefiles():
    bet_log(FILE_NAME, "Cleaning auto-generated Makefiles...")
    makefile = "Makefile"
    makefile_ext = ".make"
...
"""
def rm_xcode_files():
    bet_log(FILE_NAME, "Removing xcode workspace file...")
    sad_xcworkspace = os.path.join(PROJECT_DIR, "sad.xcworkspace")
    rdf(sad_xcworkspace)

    bet_log(FILE_NAME, "Removing xcode project files...")
